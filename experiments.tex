...
\end{tabular}  
& \begin{tabular}[c]{@{}l@{}}
  \textsl{equal-HospitalName/\textsc{hosp}}(hid, n, hid, n)\\ 
  \textsl{equal-Address/\textsc{hosp}}(hid, add, hid, add)\\ 
  \textsl{equal-City/\textsc{hosp}}(hid, c, hid, c)\\ 
  \textsl{equal-State/\textsc{hosp}}(hid, st, hid, st)\\ 
  \textsl{equal-ZipCode/\textsc{hosp}}(hid, code, hid, code)\\ 
  \textsl{equal-CountryName/\textsc{hosp}}(hid, ctr, hid, ctr)\\ 
  \textsl{equal-PhoneNumber/\textsc{hosp}}(hid, nr, hid, nr)\\ 
  \textsl{equal-MeasureName/\textsc{hosp}}(hid, mn, hid, mn)\\ 
  \textsl{equal-Condition/\textsc{hosp}}(hid, cond, hid, cond)\\ 
  \textsl{\textsc{hosp}/match-State/\textsc{ZipCode}}(hid, zid)\\ 
  \textsl{\textsc{hosp}/match-ZipCode/\textsc{ZipCode}}(hid, code)
\end{tabular} 
& 
\begin{tabular}[c]{@{}l@{}}
 \textsl{equal-Names}(id, n, id, n)\\ 
 \textsl{equal-Addr}(id, add, id, add)\\ 
 \textsl{equal-Natkey}(id, nkey, id, nkey)\\ 
 \textsl{equal-Phone}(id, ph, id, ph)\\ 
 \textsl{equal-Acc}(id, a, id, a)\\ 
 \textsl{equal-Mrkt}(id, m, id, m)\\ 
...
\multirow{2}{*}{\textsc{hosp}}  & %hosp dq rules
                        ...
                        $\mathsf{cfd_2: \textsc{hosp}([\textsl{phone}] \rightarrow [\textsl{addr, zip,state, city}],t2=(\_ \parallel \_,\_,\_,\_))}$
                        \end{tabular}                   
                        ...
                        $\mathsf{~~~~~~~~\wedge!\textsl{state}(id1, s2)~\wedge~!\textsl{state}(id2, s1)~\Rightarrow \textsl{equal-state}(id1, s1, id2, s2)}$\\
                        %second cfd:
                        $\mathsf{w_2: \textsl{zip}(id1, code)~\wedge~\textsl{zip}(id2, code)~\wedge \textsl{city}(id1, c1)~\wedge~\textsl{city}(id2, c2)~}$ \\
                        $\mathsf{~~~~~~~~\wedge!\textsl{city}(id1, c2)~\wedge~!\textsl{city}(id2, c1)~\Rightarrow \textsl{equal-city}(id1, c1, id2, c2)}$
                        \end{tabular}                 \\ 
                       & %hosp md rule
                         \begin{tabular}[c]{@{}l@{}}
